Feature: Iconomist
  ...
  Scenario: Check that iconomist options exist on the theme settings page.
    ...
    And I select the radio button "Icon" with the partial id "edit-iconomist-icons-0-rel-icon"
    And I press "Add icon"
    And I wait for AJAX to finish
    Then I should see "Path to custom icon"
    ...
    And I select the radio button "Apple Touch" with the partial id "edit-iconomist-icons-1-rel-apple-touch-icon"
